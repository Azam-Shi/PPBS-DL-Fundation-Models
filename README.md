...
| Formulation | Common metrics | Notes |
|---|---|---|
| **Partner-independent** | Precision, recall, F1, MCC, ROC-AUC, PR-AUC, area-weighted AUC | Interface residues are rare; PR-AUC and MCC are often more informative than accuracy alone. |
| **Partner-aware** | Same residue-level classification metrics, reported per partner pair | Ground truth can change depending on the supplied partner. |
| **Residue-pair contact** | Top-L, Top-L/2, Top-L/5, Top-L/10 precision | Candidate space scales with the product of chain lengths; ranking quality is central. |
| **Hot-spot / mutation-effect** | Classification: precision/recall/F1/MCC; Regression: Pearson, Spearman, RMSE, MAE | Hot-spot labels are energetic rather than purely geometric. |
...
- add a missing PPBS paper;
- add a missing code repository or webserver;
- correct a task formulation;
- update a paper link after publication;
- add benchmark datasets or evaluation protocols.

Recommended entry format:

```markdown
| **MethodName** | Year | One-sentence representation / architecture | [Paper](paper-url) | [Code](code-url) |
```
...
The survey is not public yet. After the manuscript is submitted or released, replace this placeholder with the official BibTeX entry.

```bibtex
@misc{shirali2026ppbs_survey,
  title        = {Deep Learning for Protein--Protein Binding Site Prediction: Formulations, Representations, and Learning Paradigms},
  author       = {Shirali, Azam and Garg, Ishaan and Narasimhan, Giri},
  year         = {2026},
